...
- 支持 float32 和 float64 向量输入/输出
- 支持单向量和并发批量量化/反量化（基于 goroutine）
- 可配置批量并发数（默认：runtime.NumCPU()）
- 紧凑二进制序列化（比特紧凑排列）
- 支持流式序列化，写入/读取 io.Writer / io.Reader
- 批量流式序列化/反序列化（带数量前缀帧格式）
- 码本自动缓存（线程安全，按维度和位宽索引）
- 确定性：相同种子 → 相同旋转矩阵 → 相同结果
- 纯 Go 实现，无需 CGO；大维度场景可选接入 OpenBLAS/MKL 加速
